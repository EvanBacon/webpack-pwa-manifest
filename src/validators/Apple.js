...
const Tablets = Devices.filter(({ isTablet }) => isTablet);

function assembleOrientationMedia(width, height, scale, orientation) {
  // const orientation = width > height ? 'landscape' : 'portrait';
  return `(device-width: ${Math.floor(width / scale)}px) and (device-height: ${Math.floor(
    height / scale
  )}px) and (-webkit-device-pixel-ratio: ${scale}) and (orientation: ${orientation})`;
}
